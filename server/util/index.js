...
async function existCreateQuery(config = {
    connect,
    table_name,
    read_sql,
    read_sql_config,
    ...
    if (read_sql_config !== undefined)
        if (table_name !== read_sql_config.createName)
            throw console.error('table_name 과 read_sql_config.createName이 다릅니다.')
    ...
    try {
        ...
        if (sql !== undefined)
            ...
            await connect.query(readSQL(read_sql, read_sql_config));
        correctMessage(`##{${table_name}}## create table`)
    }
}

async function existInsertQuery(config = {
    connect,
    table_name,
    read_sql,
    read_sql_config,
    ...
    const res = await connect.query(`SELECT * FROM ${table_name} WHERE ${overlap_where}`)
    ...
    if (res[0].length >= cont) {
        ...
        if (sql !== undefined)
            ...
            await connect.query(readSQL(read_sql, read_sql_config), param);
...
function correctMessage(routerName, message) {
    ...
    console.log(`${getDate()} - [${mes.result} SUCCESS] ${mes.router_path} ${routerName} `, message)
}

function errorMessage(routerName, message) {
    if (message === undefined) message = '';
    if (routerName === undefined) routerName = message
    console.error(`${getDate()} - [${mes.result} ERROR] ${mes.router_path} ${routerName} \n`, message)
}
