        ...
        if (dummyUse && dummy.length > 1) {
            for (let i = 0; i < dummy.length; i++) {
                let dmysql = sqlText.INSERT('board_basic',
                    ...
                    `(${i+1},${i+1},"${dummy[i].subject}","${dummy[i].content}",0,"${dummy[i].time}","${dummy[i].time}","${dummy[i].writer}",${dummy[i].hit},0,"normal","false")`);
...
router.post('/like', async (req, res) => {
    ...
    try {
        ...
        if (req.body.click) {
            sql = sqlText.UPDATE('likes', `hit=${req.body.hit+1}`, `type="${req.body.type}"`)
            await conn.query(sql);
